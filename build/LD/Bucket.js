...
var CBStorageBucket = (function (_Bucket) {
	...
	_createClass(CBStorageBucket, [{
		...
		key: "insertNodes",
		value: function insertNodes(triples) {
			var _this3 = this;
			...
			return jsonld.expand(triples).then(function (res) {
				_.map(res, function (val) {
					var opts = options[val["@id"]] || {};
					promises[val["@id"]] = _this3.insert(val["@id"], val, opts)["catch"](function (err) {
		...
		value: function upsertNodes(triples) {
			...
			return jsonld.expand(triples).then(function (res) {
				_.map(res, function (val) {
					var opts = options[val["@id"]] || {};
					promises[val["@id"]] = _this4.upsert(val["@id"], val, opts)["catch"](function (err) {
						return Promise.resolve(false);
					});
				});
				return Promise.props(promises);
		...
		value: function replaceNodes(triples) {
			...
			return jsonld.expand(triples).then(function (res) {
				_.map(res, function (val) {
					var opts = options[val["@id"]] || {};
					promises[val["@id"]] = _this5.replace(val["@id"], val, opts)["catch"](function (err) {
						return Promise.resolve(false);
					});
				});
				return Promise.props(promises);
		...
		value: function getNodes(subjects) {
			...
			_.map(keys, function (key) {
				promises[key] = _this6.get(key)["catch"](function (err) {
		...
		value: function removeNodes(subjects) {
			...
			var promises = {};
			_.map(keys, function (key) {
				promises[key] = _this7.remove(key)["catch"](function (err) {
					return Promise.resolve(false);
				});
			});
			return Promise.props(promises);
